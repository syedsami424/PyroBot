...
def query():
    global user_name1
    pyautogui.press('enter')
    pyautogui.write("PyroBot: What is your query?", interval=0.1)
    pyautogui.press('enter')
...
def monkey_type():
    ...
    browser.get(url)
    pyautogui.click(x=880, y=836)   #allow cookies in monkeytype site

    #pyautogui.click(x=880, y=836)
    #pyautogui.moveTo(x=41, y=477) #top left
    #pyautogui.moveTo(x=1278, y=628) #bottom right
    #w=1237
    #h=151

    time.sleep(5)
    #time.sleep(4)
    screenshot=pyautogui.screenshot(region=(41,477,1237,151))
    screenshot.save('monkeytype1.png')
    image1=Image.open('monkeytype1.png')
    image1=image1.convert("L") #grayscale conversion
    ...
    for i in range(19):
        ...
        for word in wordlist2:
            #an if condition that avoids the cursor. (| or J)
            #temp=wordlist2[word]
        ...
        if(i==19):
            ...
            break
    #pyautogui.click(x= ,y= )
''' IMPT: MAKE PYAUTOGUI CLICK ON THE TEXT FILE AFTER THIS IS DONE. '''

#######################################################################################################
'''              10fastfingers            '''
#needs some work after the task of this function gets over
def tenff():

    driver_path="./chrome-win64/chrome-win64/chrome_proxy.exe"

    chromeService=ChromeService(driver_path)
    ...
    try:
        ...
        print("Could not retrieve website, error:",str(e));
    time.sleep(7)
...
def main_menu():
    while True:
        ...
        pyautogui.press('enter')
        pyautogui.write("2. Play MonkeyType.",interval=0.1)
        pyautogui.press('enter')
        pyautogui.write("3. Play 10fastfingers.",interval=0.1)
        pyautogui.press('enter')
        pyautogui.write("4. Exit.",interval=0.1)
        ...
        if(choice=='1'):
            query()
            pyautogui.press('enter')
            pyautogui.write("PyroBot: Would you like to ask another query?",interval=0.1)
        ...
        if(choice=='2'):
            print("in choice 2 section")
            monkey_type()

        if(choice=='3'):
            print("in choice 3 section")
            tenff()
        ...
        if(choice=='4'):
            ...
            print("Invalid")
            pyautogui.write("Invalid",interval=0.1)

#########################################################################################################################################
f=open("D:\PyroBot.txt", "w")
os.startfile(r'D:\PyroBot.txt') #observation: without this, no file opens and everything will get printed onto python console
...
#pyautogui.press('enter')
pyautogui.write("Hello there, I am PyroBot. You are?",interval=0.1)
time.sleep(1)
pyautogui.press('enter')
...
pyautogui.press('enter')
if(user_name=="PyroBot"):
    pyautogui.write("lmao that is my name too", interval=0.1)
    pyautogui.press('enter')

else:
    pyautogui.write("PyroBot: Hi, "+user_name+"!!")
    pyautogui.press('enter')
    time.sleep(1)
